...
Answer: The Parameters Section. 

2. Which part of the URL can be manipulated by an attacker to cause a vulnerable web server to dump the `/etc/passwd` file? Also, name the attack used to exploit this vulnerability.

Answer: The "path" can be manipulated in the URL.
   
3. Name three threat agents that can pose a risk to your organization.

Answer: Individuals, organization or governments. 

4. What kinds of sources can act as an attack vector for injection attacks?

Answer: SQL databases and html code. Or anything that has the capability.
...
Answer: Input sanitation is the process by which the web application (preferably server-side) actively        modifies user input to an acceptable format or blocks the user's request.

2. Name the process that tests user and application-supplied input. The process is designed to prevent malformed data from entering a data information system by verifying user input meets a specific set of criteria (i.e. a string that does not contain standalone single quotation marks).

Answer: Input validation is the process that checks if a user's input is in an acceptable format.
...
Answer: The robots.txt file contains sensitive web server information, such as a list of files and web server directories, that website owners want to hide from web crawlers.
...
False. Client-side validation is more vulnerable to bypass and manipulate.  
...
1. Define all four factors of multifactor authentication and give examples of each:
   ...
   Example: ID Card, Door Key
   
   - Factor 3: Biometrics

   Example: Facial Recognition, Fingerprint, IRIS Scan
...
Answer: An interface that is restricted based on priviledges. 
